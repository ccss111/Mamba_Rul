...
MAMBA_TEST_ROOT = os.path.dirname(os.path.abspath(__file__))
if MAMBA_TEST_ROOT not in sys.path:
    sys.path.insert(0, MAMBA_TEST_ROOT)

from dataset import *
from mamba_ED3kgat_parallel import MambaED3KGATRegressor
from utils import *
...
if __name__ == '__main__':
    ...
    for num in seed_sequence:
        ...
        model = MambaED3KGATRegressor(
            input_size=args.feature_num,
            sequence_len=args.sequence_len,
            lstm_hidden_dim=encoder_hidden_size,
            lstm_num_layers=2,
        ...
        with open(log_path, "a", encoding="utf-8") as f:
            ...
            f.write(f"模型结构(model_structure): {type(model).__name__}\n")
            f.write(f"学习率: {args.lr}\n")
            f.write("学习率调度器: step\n")
            f.write(f"Step步长(step_size): {args.step_size}\n")
            f.write(f"Step衰减系数(gamma): {args.gamma}\n")

            f.write("编码器: Mamba + LSTM + GAT\n")
            f.write(f"Mamba层数: {mamba_num_layers}\n")
        ...
        auto_test_cmd = [
            sys.executable,
            os.path.join(parent_dir, 'mamba_ED3kgat_test', 'test_model.py'),
