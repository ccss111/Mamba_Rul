...
class MambaBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        if not x.is_cuda:
            raise RuntimeError("MambaBlock requires CUDA tensors; CPU fallback is disabled.")
        x = self.mamba(x)
        x = self.dropout(x)
        return residual + x
...
class EncoderDecoder(nn.Module):
    def __init__(self,
        ...
        encoder_input_size = int(getattr(self.encoder.lstm, 'input_size', 0))
        if self.use_spatial_gat:
            mamba_input_size = int(gat_out_dim)
        else:
            mamba_input_size = encoder_input_size
        if mamba_input_size <= 0:
            raise ValueError("failed to infer Mamba input size")

        self.mamba_blocks = self._build_mamba_blocks(mamba_input_size)
        self.encoder_input_projection = (
            nn.Identity()
            if mamba_input_size == encoder_input_size
            else nn.Linear(mamba_input_size, encoder_input_size)
        )
    ...
    def _ensure_pre_encoder_modules(self, encoder_x):
        if not hasattr(self, 'mamba_blocks') or self.mamba_blocks is None:
            self.mamba_blocks = self._build_mamba_blocks(int(encoder_x.size(-1)))
        if not hasattr(self, 'encoder_input_projection'):
            encoder_input_size = int(getattr(self.encoder.lstm, 'input_size', encoder_x.size(-1)))
            self.encoder_input_projection = (
                nn.Identity()
                if int(encoder_x.size(-1)) == encoder_input_size
                else nn.Linear(int(encoder_x.size(-1)), encoder_input_size)
            )
        if not hasattr(self, 'encoder_regressor'):
            encoder_hidden_size = getattr(self.encoder.lstm, 'hidden_size', int(encoder_x.size(-1)))
            self.encoder_regressor = nn.Linear(int(encoder_hidden_size), 1)
    ...
    def forward(self, encoder_x):
        ...
        # GAT后、encoder-lstm前的Mamba时序增强
        self._ensure_pre_encoder_modules(encoder_x)
        encoder_x = self._apply_mamba_blocks(encoder_x)
        encoder_x = self.encoder_input_projection(encoder_x)
